...
export const InteractiveMap: React.FC<InteractiveMapProps> = ({ companies }) => {
  ...
  useEffect(() => {
    ...
    // Centro inicial padrão (São Paulo)
  ...
  useEffect(() => {
    ...
    companies.forEach(company => {
      const markerColor = company.status === 'Ativo' ? '#2563eb' : '#94a3b8';
      
      const nextContactFormatted = company.nextContactDate 
        ? new Date(company.nextContactDate).toLocaleDateString('pt-BR') 
        : null;

      const markerHtml = `
        <div style="position: relative; width: 38px; height: 38px; filter: drop-shadow(0 4px 6px rgba(0,0,0,0.2));">
    ...
    // Só ajusta o zoom se não houver busca ativa
    if (companies.length > 0 && mapRef.current && !searchMarkerRef.current) {
  ...
  return (
    <div className="relative w-full h-full bg-slate-100 z-0 flex flex-col">
      {/* Floating Search Bar */}
      <div className="absolute top-6 left-1/2 -translate-x-1/2 z-[1001] w-full max-w-lg px-4 no-print">
        <form onSubmit={handleSearch} className="relative group">
          <input 
            type="text" 
            placeholder="Buscar endereço ou CEP para prospecção..." 
            className="w-full h-14 pl-14 pr-12 bg-white/90 backdrop-blur-md border border-white/50 rounded-2xl shadow-2xl outline-none focus:ring-4 focus:ring-blue-500/20 focus:bg-white transition-all text-slate-700 font-medium"
            value={searchTerm}
          ...
          {searchTerm && (
            <button 
              type="button" 
              onClick={clearSearch}
              className="absolute right-4 top-1/2 -translate-y-1/2 w-6 h-6 flex items-center justify-center bg-slate-100 text-slate-500 rounded-full hover:bg-slate-200 transition-colors"
            >
              ✕
            </button>
          )}
